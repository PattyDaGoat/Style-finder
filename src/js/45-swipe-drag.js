...
const SW={
  commitPx:  20,     /* travel that always commits — barely more than a click */
  commitFrac: 0.07,  /* …or this share of the card's width, whichever is SMALLER.
  ...
  commitV:   0.14,   /* px/ms — the gentlest wrist flick clears this */
  flickMinPx: 12,    /* …but a flick still has to have gone somewhere. Velocity
                        ...
                        comes down slower than everything else. It cannot go
                        below TAP_SLOP (6) or a plain click becomes a swipe —
                        which is the bug this whole thread started with. 12px is
                        double that, and about as far as this can sensibly go. */
  vMax:       3,     /* px/ms ceiling, so one bad frame can't dominate */
  dtMin:      4,     /* ms floor, so we never divide by ~0 */
  upPx:      30,     /* upward travel for a love */
  upV:       0.18,
