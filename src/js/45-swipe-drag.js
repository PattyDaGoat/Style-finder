...
const SW={
  commitPx:  95,     /* travel that always commits */
  commitV:   0.55,   /* px/ms — a flick this fast commits… */
  flickMinPx: 50,    /* …but only once it has actually gone somewhere. Velocity
  ...
  upPx:     110,     /* upward travel for a love */
  upV:       0.65,
  maxRot:    16,     /* degrees at a full card-width of travel */
  lift:    1.03,     /* scale while held: the card comes off the stack */
  flyMs:    380,
  springMs: 520
};
...
function attachDrag(card){
  ...
  const setStamps=(dx,dy)=>{
    ...
    if(love)love.style.opacity=up?Math.min(1,(-dy-40)/80):0;
    if(like)like.style.opacity=(!up&&dx>0)?Math.min(1,(dx-16)/80):0;
    if(nope)nope.style.opacity=(!up&&dx<0)?Math.min(1,(-dx-16)/80):0;};
